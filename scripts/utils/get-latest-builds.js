...
function convertFilename(filename, channel) {
    ...
    const [_game, release, version, change] =
        removeFileExtension(filename).split("-");
    ...
    return {
        ...
        stem:
            removeFileExtension(filename) + "-" + channel.replaceAll("-", "_"),
        filename: filename,
    };
}
...
export default async function getLatestBuilds() {
    ...
    for (const channel of channels) {
        channelsWithBuilds[channel] = fs
            ...
            .filter(
                (filename) =>
                    !filename.startsWith(".") && filename.endsWith(".ini")
            )
            .map((filename) => convertFilename(filename, channel))
            .sort(sort);

        channelsWithBuilds[channel].base = fs.existsSync(
            "translations/" + channel + "/.base"
        ); // If exists, this channel will be the baseline version for all differences

        if (fs.existsSync("translations/" + channel + "/.changenum")) {
            channelsWithBuilds[channel] = channelsWithBuilds[channel].sort(
                (a, b) => {
                    return b.change - a.change;
                }
            );
